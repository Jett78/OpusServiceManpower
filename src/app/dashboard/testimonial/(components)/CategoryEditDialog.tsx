...
export default function CategoryEditDialog({ id, setRefreshNow }: Props) {
  ...
  useEffect(() => {
    const fetch = async () => {
      setIsFetching(true);
      const { data, error, status } = await supabase.from("Testimonial").select().eq("id", id).single();
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    setIsUpdating(true);
    const { data, error, status } = await supabase.from("Testimonial").update(values).eq("id", id);
          ...
          >
            ...
            <div className=" flex justify-end mt-8">
              <Button
                ...
                {isUpdating ? " Please wait" : " Update Testimonial"}
              </Button>
            </div>
          </form>
        </Form>
