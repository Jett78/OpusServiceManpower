"use client";
import { Group, Home, Inbox, LayoutDashboard, ListOrdered, LogOut, Newspaper, Shapes, User2, Users } from "lucide-react";
      ...
      <div className=" flex ">
        <div className=" flex flex-col justify-between h-screen w-2/12 bg-primary-foreground  bg-primary-500 ">
          <div>
            ...
            /> */}
            <p className=" text-2xl font-semibold tracking-wider py-2 ">Opus</p>
            <Separator />
            ...
            <div className=" space-y-2 pt-4">
              {navItems.map((item: any, index: number) => (
                <div
                  ...
                  <Link
                    href={item.href}
                    className={`link ${pathname === item.href ? "bg-secondary" : ""}  py-1.5 px-6 hover:text-secondary-950 duration-100  opacity-95 flex items-center gap-1  `}>
                    {item.icon} {item.name}
                  </Link>
                </div>
              ))}
